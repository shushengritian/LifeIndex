...
import type { BackupData, LifeIndexBackupV3 } from '@/shared/domain/types'
...
export interface BackupPreview {
  ...
  appVersion: string
  exportedAt: string
  counts: LifeIndexBackupV3['counts']
  expiresAt: string
}
...
export class BackupService {
  ...
  async createSnapshot(locale = navigator.language || 'zh-CN'): Promise<LifeIndexBackupV3> {
    logger.info('backup.export.started', { operation: 'snapshot', formatVersion: 3 })

    try {
      const data = await this.readConsistentData()
      const now = this.clock.now()
      const backup: LifeIndexBackupV3 = {
  ...
  serialize(backup: LifeIndexBackupV3): string {
  ...
  inspectText(text: string, byteLength = new TextEncoder().encode(text).byteLength): BackupPreview {
    logger.info('backup.import.inspectionstarted', { operation: 'inspect', formatVersion: 3 })
  ...
  async restore(token: string): Promise<RestoreResult> {
    logger.info('backup.restore.started', { operation: 'replace', formatVersion: 3 })
  ...
  private async replaceData(backup: LifeIndexBackupV3): Promise<void> {
