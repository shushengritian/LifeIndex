...
test('cessation preserves confirmations, handles smoking and craving, and persists across reload', async ({
  ...
  await page.getByRole('button', { name: '编辑吸烟记录', exact: true }).click()
  await page.getByRole('button', { name: '删除记录', exact: true }).click()
  await page
    .getByRole('dialog', { name: '删除戒烟记录？' })
    .getByRole('button', { name: '删除记录' })
    .click()
  await expect(page.getByRole('status')).toContainText('未自动创建无烟确认')
  await page.getByRole('button', { name: '记录戒烟事件', exact: true }).click()
  await page.getByRole('button', { name: '记录烟瘾', exact: true }).click()
  ...
  await page.getByRole('button', { name: '缓解了', exact: true }).click()
  await page.getByRole('button', { name: '保存烟瘾记录', exact: true }).click()
...
test('future plans can be cancelled; hiding keeps history and the Health add menu restores access', async ({
  ...
  await page
    .getByRole('dialog', { name: '取消未来计划？' })
    .getByRole('button', { name: '确认取消计划' })
    .click()
