...
2026-09-04 发布验收补充：用户明确接受当前 V1 交付，并同意将未完成的 iPhone 真机验收延期。该一次性发布门槛调整记录于 [ADR-0005](docs/adr/0005-v1-owner-acceptance.md)；产品范围、Local-First 架构和数据安全要求不变，延期测试不得写为已通过。
...
2026-09-20：用户要求本轮间距与专注显示优化后发布 3.0.0 可用产品里程碑（ADR-0019）。这是应用版本升级，数据仍 V4，Local-First 产品范围不变；实时已专注/剩余/目标分开表达，历史汇总只计已保存的完成会话。部署状态以 PLAN.md 为准。

2026-09-20：2.2.1/V4 已部署，设备验收按 ADR-0015 进行；当前产品范围按 ADR-0017 收敛，见 PLAN.md。
...
2026-09-14 发布授权补充：用户确认本地无需要备份的数据并要求直接发布 V2.1，[ADR-0010](docs/adr/0010-v2.1-publication-authorization.md) 豁免本次空备份前置要求，保留自动化门槛；真实 iPhone 专项作为发布后待确认，不记为通过。
