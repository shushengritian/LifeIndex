...
> 2026-09-20 部署完成：2.2.0/V4 已上线，源码 3f2f670，Pages 35453823388 构建/部署/线上验证全部成功；234 项单元/集成、63 项浏览器通过/1 原有跳过，独立线上复验 13 通过/1 原有跳过。后续按 ADR-0015 在手机验收，原生快捷模板交付与同账本验证仍待办；不把 App 发布等同长期目标全部完成。

> 2026-09-20 新授权：用户确认“先部署，再手机验证”，见 ADR-0015。Mac 原生测试不再阻塞本次 App 发布。冻结 2.2.0/V4，按现有 Pages 全量门槛发布；快捷原生产物和 iPhone 验收仍待完成，不据此宣称整项目完成。下方受阻状态已被此授权替代。

> 2026-09-20 收口状态：22314ae 的 [CI 35453150381](https://github.com/shushengritian/LifeIndex/actions/runs/35453150381) 全部成功，日志确认 234 项单元/集成、63 项浏览器通过及 1 原有跳过。没有运行中的 CI 需要继续轮询。原生快捷安装许可、iOS 版本/使用入口及设备验收协作连续多轮待回复，目标在此受阻，停止自动重试；原生模板、剩余验收、版本冻结和正式部署仍待完成，不缩减范围、不视为发布。恢复时先取得验收所需确认，再按 G5 审计继续。
