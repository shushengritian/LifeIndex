...
- Current allowlisted types are `check-habit` and `start-focus`.
...
- Durable receipts protect against repeated link execution across app restarts.

## Consequences

- Action receipts become a small V1 database store and are included in full backup/restore.
- Link callers must generate a UUID for each intended action.
...
- In-memory deduplication: lost on app restart and insufficient for link retries.
