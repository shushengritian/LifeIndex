...
专注自然到时由 `useFocusCompletion` 调用 `FocusRepository.reconcileActive` 保存，结束时间使用原计划终点，不使用延迟唤醒时间。提前结束需用户确认后保存；取消本次不计入完成汇总。自然完成写入失败时保留 active 记录及固定终点，显示重试入口，不每秒重复写入。
